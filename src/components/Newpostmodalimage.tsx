...
const Newpostmodalimage: React.FC<NewpostmodalimageProps> = ({ average, imagesNum, myIndex }) => {
    ...
    useEffect(() => {
        console.log(location[myLocation[myIndex]])
    }, [myLocation])

    return (
    <>
        {location[myLocation[myIndex]] !== undefined ?
            <motion.div layout style={{backgroundColor : average[myIndex], gridRowStart : location[myLocation[myIndex]][0]}} className={`col-start-${location[myLocation[myIndex]][1]} relative w-full h-full flex items-center justify-center`}>
                <motion.div className='absolute w-full h-full opacity-0 hover:opacity-100 hover:bg-black hover:bg-opacity-80 flex justify-center items-center'>
                    <div className='w-1/2 h-1/2 grid grid-cols-3 gap-2 items-center'>
                        {tmp.map((data, index) => (
                            <div key={data} className="w-5 h-5 border flex items-center justify-center cursor-pointer"
                                onClick={() => {
